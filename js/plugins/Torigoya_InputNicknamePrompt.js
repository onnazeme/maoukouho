//=============================================================================
// Torigoya_InputNicknamePrompt.js(Nickname版へ改造)
//=============================================================================
 ...
 *
 * Plugin Command:
 *   InputNicknamePrompt 1      # display input name prompt of Actor ID: 1
 *   InputNicknamePrompt 1 100  # display input name prompt of Actor ID: 1 (max length: 100)
 *   InputNicknamePrompt 1 100 Please, input your name.  # display input name prompt with custom message
 */
 ...
 *
 * プラグインコマンド:
 *   InputNicknamePrompt 1      # アクター1番の名前入力ダイアログを表示する
 *   InputNicknamePrompt 1 100  # アクター1番の名前入力ダイアログを表示する (最大 100 文字)
 *   InputNicknamePrompt 1 100 名前を入力するのじゃ  # メッセージの変更
 */

(function (global) {
    'use strict';

    var PLUGIN_NAME = 'Torigoya_InputNicknamePrompt';
    ...
    var InputNicknamePrompt = {
        name: PLUGIN_NAME,
        settings: settings
    };

    InputNicknamePrompt.runCommand = function (args) {
        var actor = $gameActors.actor(~~args.shift());
        var max = Number(args.shift());
        if (isNaN(max)) max = InputNicknamePrompt.settings.maxLength;
        var message = args.join(' ') || InputNicknamePrompt.settings.getMessage();
        message += '\n' + InputNicknamePrompt.settings.getMaximumMessage().replace(/%1/g, '' + max);
        var name = window.prompt(message, actor.nickname());
        if (name) {
            name = name.trim();
            if (name.length > 0) {
                actor.setNickname(name.substr(0, max));
            }
        }
    };

    InputNicknamePrompt.afterCommand = function () {
    ...
    Game_Interpreter.prototype.pluginCommand = function (command, args) {
        if (command === 'InputNicknamePrompt') {
            InputNicknamePrompt.runCommand(args);
            InputNicknamePrompt.afterCommand();
            return true;
        }
    ...
    global.Torigoya.InputNicknamePrompt = InputNicknamePrompt;
})(this);
